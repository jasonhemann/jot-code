...
(define c->L (lambda (c) (cadddr (cdr c))))

(define c->F (lambda (c) (cadddr (cddr c))))

(define c->N (lambda (c) (cadddr (cdddr c))))
...
(define make-tag-A
  (lambda (tag pred)
    (lambda (u)
      (lambdag@ (c : S D A T L F N)
        (case-value (walk u S)
          ((x) (cond
                 ((make-tag-A+ x tag pred c S D A T L F N) =>
                  unit)
                 (else (mzero))))
          ((au du) (mzero))
          ((u) (cond
...
(define make-tag-A+
  (lambda (u tag pred c S D A T L F N)
    (cond
       ...
       (lambda (A+)  
         (cond
           ...
           (else (let ((D (subsume A+ D))
                       (A (append A+ A)))
                   (subsume-A S D A T L F N))))))
      (else #f))))

(define subsume-A
  (lambda (S D A T L F N)
    (let ((x* (rem-dups (map lhs A))))
      (subsume-A+ x* S D A T L F N))))

(define subsume-A+
  (lambda (x* S D A T L F N)
    (cond
      ((null? x*) `(,S ,D ,A ,T ,L ,F ,N))
      (else (let ((x (car x*)))
...
(define =/= 
  (lambda (u v)
    (lambdag@ (c : S D A T L F N)
      (cond
        ((unify u v S) => (post-unify-=/= S D A T L F N))
        (else (unit c))))))
...
(define == 
  (lambda (u v)
    (lambdag@ (c : S D A T L F N)
      (cond
        ((unify u v S) =>
         (post-unify-== c S D A T L F N))
        (else (mzero))))))

(define post-unify-==
  (lambda (c S D A T L F N)
    (lambda (S+)
      (cond
        ((eq? S+ S) (unit c))
        ((verify-D D S+) =>
         (lambda (D)
           (cond
             ((post-verify-D S+ D A T L F N) => unit)
             (else (mzero)))))
        (else (mzero))))))
...
(define post-verify-A
  (lambda (S D T L F N)
    (lambda (A)
      (let ((D (subsume A D)))
        (cond
          ((verify-T T S) => (post-verify-T S D A L F N))
          (else #f))))))
...
(define post-verify-T
  (lambda (S D A L F N)
    (lambda (T)
      (subsume-T T S (subsume T D) A '() L F N))))

(define subsume-T  
  (lambda (T+ S D A T L F N)
    (let ((x* (rem-dups (map lhs A))))
      (subsume-T+ x* T+ S D A T L F N))))

(define subsume-T+ 
  (lambda (x* T+ S D A T L F N)
    (cond
      ((null? x*)
       (let ((T (append T+ T)))
         `(,S ,D ,A ,T ,L ,F ,N)))
  ...
  (lambda (tag u)
    (cond
      ...
      (else
       (lambdag@ (c : S D A T L F N)
         (cond
           ((absento+ u tag c S D A T L F N) => unit)
           (else (mzero))))))))

(define absento+  
  (lambda (u tag c S D A T L F N)
    (case-value (walk u S)
      ((x)
       (let ((T+ (ext-T x tag S T)))
         (cond
           ...
           (else
            (let ((D (subsume T+ D)))
              (subsume-T T+ S D A T L F N))))))
      ((au du)
       (let ((c (absento+ au tag c S D A T L F N)))
         (and c
           (let ((S (c->S c))
                 (D (c->D c))
                 (A (c->A c))
                 ...
                 (F (c->F c))
                 (N (c->N c)))
             (absento+ du tag c S D A T L F N)))))
      ((u)
       (cond
